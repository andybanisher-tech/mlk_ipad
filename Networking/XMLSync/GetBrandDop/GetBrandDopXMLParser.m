...
@interface GetBrandDopXMLParser () <NSXMLParserDelegate>
...
@implementation GetBrandDopXMLParser
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName
  namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qualifiedName
	...
	if ([elementName isEqualToString:@"m:GetBrandDopResponse"]) {
...
- (void)createBrand {
    if (sqlite3_open(SQLWorker.dbPath.UTF8String, &database) == SQLITE_OK) {
        ...
        for (int y = 0; y < self.brands.count; y++) {
            ...
            const char *sql = "insert into BrandDop (BrandId, BrandName) Values(? , ?)";
...
- (void)deleteBrand {
    if (sqlite3_open(SQLWorker.dbPath.UTF8String, &database) == SQLITE_OK) {
        ...
        sqlite3_exec(database, [[NSString stringWithFormat:
                                 @"delete from BrandDop"] UTF8String], NULL, NULL, NULL);
